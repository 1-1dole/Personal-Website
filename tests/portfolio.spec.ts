...
test('filters work and content remains without JavaScript', async ({ page }) => { await page.goto('./'); const filters = page.getByRole('button', { name: /Filter projects by/i }); await expect(filters.first()).toBeVisible(); const before = await page.locator('[data-project-card], article').count(); await filters.last().click(); const after = await page.locator('[data-project-card]:visible, article:visible').count(); expect(after).not.toBe(before); const context = await page.context().browser()!.newContext({ javaScriptEnabled: false }); const noJs = await context.newPage(); await noJs.goto('http://127.0.0.1:4321/Personal-Website/'); await expect(noJs.getByRole('heading', { name: 'Human Nutrition Unit', exact: true })).toBeVisible(); await context.close(); });
test('mobile navigation Escape selection focus at 320/375', async ({ page }) => { for (const width of [320, 375]) { await page.setViewportSize({ width, height: 800 }); await page.goto('./'); const button = page.getByRole('button', { name: 'Open navigation' }); await button.click(); const navigation = page.getByRole('navigation', { name: /primary/i }); await expect(navigation).toHaveAttribute('data-open', 'true'); await page.keyboard.press('Escape'); await expect(button).toBeFocused(); await button.click(); await navigation.getByRole('link', { name: 'Work', exact: true }).click(); await expect(button).toBeFocused(); } });
...
test('case page command palette routes back to homepage sections', async ({ page }) => { await page.goto('projects/human-nutrition-unit/'); const opener = page.getByRole('button', { name: 'Open command palette' }); await opener.click(); const dialog = page.getByRole('dialog', { name: /command palette/i }); await expect(dialog).toBeVisible(); await expect(dialog.getByRole('link', { name: 'Work' })).toHaveAttribute('href', '/Personal-Website/#work'); await dialog.getByRole('link', { name: 'Work' }).click(); await expect(page).toHaveURL(/\/Personal-Website\/#work$/); });
